...
type AgeCategory = "teens" | "twenties" | "thirties" | "forties";
...
interface PreviousOutfit {
  items?: string[];
}

export class OutfitService {
  private static readonly clothingDatabase: ClothingDatabase = {
    casual: {
      cold: {
        tops: [
          "厚手のニット",
  ...
  private static readonly colorPalettes: ColorPalettes = {
  ...
  static async generateOutfit(
    ...
    previousOutfits: PreviousOutfit[] = []
    ...
    try {
      ...
      // 性別と年齢に基づいてアイテムを調整
      const adjustItemsForDemographics = (
        items: string[],
      ...
      const getRandomItem = (items: string[], avoid: string[] = []): string => {
      ...
      const mainColor = colors?.[Math.floor(Math.random() * colors.length)] || "ブラック";
      ...
      // コーディネート説明の生成
      let description = `${temperature}°C、${weather}の日におすすめの${
        styleDescriptions[style] || styleDescriptions.casual
      }です。`;
